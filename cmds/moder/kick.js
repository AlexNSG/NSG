...
module.exports.run = async (bot, message, args) => {
    ...
    if(!member.kickable) 
    return message.reply("Вы не можете кикнуть пользователя, который выше или имеет ту же роль, что и вы.");
    ...
    .catch(error => message.reply(`Извините ${message.author} Я не мог кикнуть из-за : ${error}`));
    message.channel.send(`${member.user} кикнут администратором ${message.author} причина: ${reason}`).then(msg => msg.delete(5000)).then(message.delete(5000));
    logschannels.send(kickEmbed);
}
